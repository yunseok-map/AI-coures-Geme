...
export const workTerms = [
  // ===================== 챕터 3 — 일 시키기 =====================
  {
    term: 'CLAUDE.md · AGENTS.md', en: 'Instruction files', chapter: 3,
    analogy: '신입에게 주는 업무 지침서. 매번 읽고 시작한다',
    explain: '프로젝트 폴더에 두는 지침 파일이다. 일을 시작할 때마다 자동으로 읽힌다. 규칙, 하지 말 것, 우리 팀 관행을 적는다. Claude Code 는 CLAUDE.md 를 읽고, 다른 코딩 에이전트들은 AGENTS.md 를 읽는다 — 목적은 같고 이름이 다르다.',
  ...
  },
  {
    ...
    analogy: '말만 하지 않고 실제로 연장을 집어드는 것',
  ...
  },
  {
    ...
    analogy: '한 사람의 부하 직원이냐, 각자 자기 자리를 가진 동료들이냐',
  ...
  },
  {
    ...
    explain: '여러 에이전트의 순서·병렬·합류를 설계하는 것이다. 대표 패턴 셋 — 코디네이터(한 명이 배분·취합), 팬아웃(같은 일을 여럿에게 동시에), 파이프라인(줄줄이 넘기기).',
  ...
  },
  {
    ...
    explain: '큰 탐색 작업을 서브에이전트의 별도 책상에서 돌려, 본 대화의 책상을 깨끗하게 유지하는 기법이다. 컨텍스트 로트를 피하는 가장 실전적인 방법이다.',
  ...
  },
  {
    ...
    explain: '자동으로 흘러가는 과정 중 사람의 승인이 있어야 다음으로 넘어가는 지점을 두는 설계다. 어디에 둘지가 설계의 전부다. 너무 많으면 자동화가 무의미하고, 없으면 사고가 난다.',
  ...
  },
  {
    ...
    example: '긴 작업 중 "아까 말한 규칙"을 AI가 잊는 현상의 정체',
    myth: '"AI가 까먹었다"가 아니라 요약되면서 잘렸다. 중요한 규칙을 파일에 적어야 하는 이유',
    source: D
  },
  {
    term: '체크포인트', en: 'Checkpoint', chapter: 3,
    analogy: '게임 세이브 포인트',
    explain: '지시를 보낼 때마다 만들어지는 복원 지점이다. 파일을 고치기 전 상태를 저장해 되돌릴 수 있다. 코드만, 대화만, 또는 둘 다 되돌릴 수 있다.',
  ...
  },
  {
    ...
    explain: 'AI가 해서는 안 되는 행동을 구조적으로 막는 장치다. 권한 규칙, 승인 절차, 금지 목록이 여기 해당한다. 지시로 부탁하는 게 아니라 못 하게 만드는 게 요점이다.',
  ...
  },
  {
    ...
    analogy: '서류 사이에 몰래 끼워 넣은 가짜 지시서를 직원이 그대로 따르는 것',
  ...
  },
  {
    ...
    explain: '워크슬롭이 직장 안의 문제라면 AI 슬롭은 인터넷 전반의 대량 생성 저품질 콘텐츠를 가리킨다. 검색 결과와 자료의 신뢰도를 떨어뜨린다.',
  ...
  },
  {
    ...
    example: 'AI가 만든 표 30개를 검토 없이 쓰다가 한 달 뒤 근본 수치가 틀린 걸 발견',
  ...
  },
  {
    ...
    myth: '"지우면 된다"가 아니다. 이미 전송된 자료의 처리 방식은 우리가 통제하지 못한다',
  ...
  },
  {
    ...
    explain: '도구 변화 속도, 늘 배워야 한다는 압박, 그리고 늘어난 검증 노동이 겹쳐 생기는 피로다. 워크슬롭을 받는 쪽에서 특히 심하다.',
  ...
  },
  {
    ...
    explain: '결과물을 직접 뜯어보지 않고 AI에게 맡겨 겉으로 보이는 동작만 확인하며 진행하는 방식이다. 시제품을 빨리 만들 때는 강력하지만, 문제가 생겼을 때 아무도 원인을 모른다는 대가가 따른다.',
    example: '사내 간단 도구를 하루 만에 만들기 — 대신 유지보수 계획은 별도로 필요',
  ...
  },
  {
    ...
    analogy: '만들기 전에 "무엇을 만들 것인가"를 문서로 합의하는 것',
    ...
    myth: '"옛날 방식"이 아니다. AI 시대에 오히려 다시 쓰이는 이유가 위와 같다',
