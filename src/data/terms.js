...
const coreTerms = [
  ...
  },
  {
    ...
    explain: '한 번 일할 때 AI가 동시에 볼 수 있는 양이다. 대화 기록, 첨부 파일, 지침이 전부 이 안에 들어간다. 꽉 차면 오래된 것부터 밀려나거나 요약된다.',
    example: '긴 회의록 10개를 한 대화에 넣으면 앞쪽 내용을 AI가 놓치기 시작한다',
    myth: '"크면 무조건 좋다"가 아니다. 컨텍스트 로트를 같이 보자',
    source: D
  },
  {
    term: '컨텍스트 로트', en: 'Context Rot', chapter: 1,
    analogy: '책상에 서류를 쌓을수록 가운데 낀 종이를 못 찾는다',
  ...
  },
  {
    ...
    analogy: '모르는 걸 모른다고 못 하고 그럴듯하게 지어낸다',
    explain: 'AI가 사실이 아닌 내용, 또는 준 자료와 어긋나는 내용을 지어낸다. 문장이 자연스러울수록 알아채기 어렵다. 말투가 확신에 차 있어도 내용이 맞다는 뜻은 아니다.',
    example: '없는 법 조항 번호, 없는 논문 제목, 틀린 통계를 아주 정확한 형식으로 내놓는다',
  ...
  },
  {
    ...
    explain: '대화가 시작되기 전에 AI에게 미리 건네는 기본 지침이다. 역할, 말투, 하지 말아야 할 것이 여기 들어간다. 우리가 매번 치는 질문과는 층이 다르다.',
  ...
  },
  {
    ...
    analogy: '지시문 다듬기를 넘어, 책상 위에 무엇을 올려둘지 챙기는 일',
    explain: '2025년부터 프롬프트 엔지니어링의 다음 단계로 통한다. 무엇을 넣을지보다 무엇을 빼야 정확해지는지가 중요하다. 컨텍스트 로트 때문에 "다 넣기"는 실패한다.',
  ...
  },
  {
    ...
    analogy: '시험 볼 때 외운 걸로 답하지 말고 자료집을 펴서 답하게 한다',
  ...
  },
  {
    ...
    example: '종이 영수증 사진 20장을 찍어 넣고 엑셀 표로 정리한다',
    myth: '"이미지를 만들어 주는 것"이 아니다. 그건 이미지 생성. 멀티모달은 읽고 이해하는 쪽',
    source: 'https://platform.claude.com/docs/en/build-with-claude/vision'
  },
  {
    term: '추론 모델', en: 'Reasoning Model', chapter: 1,
    analogy: '바로 답하지 않고 종이에 풀이를 써 본 뒤에 답한다',
    explain: '답하기 전에 한 단계씩 생각을 거치는 모델이다. 느리고 비싸지만 복잡한 문제에서 정확도가 오른다. 얼마나 오래 생각할지를 노력 수준으로 조절한다.',
  ...
  },
  {
    ...
    analogy: '같은 은행을 창구에서도, ATM에서도, 앱에서도 쓴다',
  ...
  },
  {
    ...
    explain: '공식 정의는 "각자의 대화 기록과 지식 베이스를 가진 독립된 작업 공간"이다. 안에 지침·자료·대화가 함께 들어간다. 대화마다 배경을 다시 설명하지 않아도 되는 게 제일 크다.',
  ...
  },
  {
    ...
    explain: '대화가 아니라 결과물이다. 문서·표·코드·다이어그램·작동하는 웹앱이 대화창 옆 별도 창에 뜨고, 거기서 계속 고칠 수 있다. 공식 기준은 "분량이 있고 독립적이며, 대화 밖으로 가져가 쓸 만한 것".',
  ...
  },
  {
    ...
    explain: '한 회사의 제품 이름이 아니라 도구의 한 갈래다. 터미널(명령창)에 설치하고 말로 지시하면 내 컴퓨터의 파일을 실제로 읽고 고치고 명령을 실행한다. 채팅과 갈리는 지점이 여기다. 회사마다 하나씩 있고(claude · codex · gemini) 공식 소개 문장이 서로 거의 같다.',
    example: '터미널에 이름을 치고 들어가 폴더 안 문서 200개의 머리말을 한 번에 통일한다',
    myth: '"이름에 Code가 있으니 개발자 전용?" 공식 정의는 셋 다 코딩 도구가 맞다. 다만 하는 일이 파일과 명령을 다루는 것이라 코딩 아닌 업무에도 쓰이기 시작했다. 하나를 익히면 나머지도 조작이 같다. 다른 건 지침 파일 이름뿐이다 (CLAUDE.md / AGENTS.md / GEMINI.md)',
  ...
  },
  {
    ...
    explain: '공식 소개는 "어디서든 방향을 잡아줄 수 있는 작업 완수 도구. 목표를 주면 내 파일과 도구를 넘나들며 일한다"이다. CLI 에이전트와 같은 기술을 쓰되 터미널 대신 일반 화면에서 돈다. 개발자가 아닌 사람의 사무 업무를 겨냥해 만들었다. 데스크톱·웹·모바일에서 돌아간다.',
    example: '여러 자료를 대조해 표를 만들고, 계약서를 무더기로 검토하고, 정기 보고서를 반복해서 낸다',
  ...
  },
  {
    ...
    explain: 'OpenAI의 코딩 에이전트 제품이다. 터미널에서 시작해 지금은 ChatGPT 앱·웹, IDE 확장, 클라우드까지 같은 에이전트가 붙는다. 공식 한 줄 소개가 "터미널에서 코드를 살펴보고 고치고 실행한다"이다. 클라우드에 일을 맡겨 두고 자리를 비웠다가 결과만 받을 수도 있다. 지침 파일은 AGENTS.md다.',
    example: '프로젝트 폴더를 통째로 맡겨 두고 자리를 비운 뒤 결과만 받는다',
    myth: '"Codex는 모델 이름 아닌가?" 한 이름이 두 층에 다 붙어 있어서 헷갈린다. 사람이 설치해서 쓰는 Codex는 제품이고, 그 안에서 도는 것은 GPT 계열 모델이다. 코딩에 맞춘 모델 갈래에도 Codex라는 이름이 붙어 모델 목록에 같이 나온다',
    source: 'https://learn.chatgpt.com/docs/codex/cli'
  },
  {
    term: '커맨드', en: 'Command', chapter: 2,
    analogy: '자주 쓰는 지시를 단축키로 등록해둔 것',
    explain: '프롬프트에 `/이름`을 쳐서 부르는 재사용 지시다. 기본 커맨드가 있고 직접 만들 수도 있다. 공식 문서가 "슬래시 커맨드"에서 "슬래시"를 뺐고, 직접 만드는 커맨드 자리는 스킬이 가져갔다(전에 만든 파일은 계속 돈다).',
    example: '`/주간보고`를 치면 정해둔 형식으로 초안이 나온다',
    myth: '"커맨드 = 스킬"? 둘이 점점 가까워지고 있다. 다만 스킬은 AI가 알아서 꺼내 쓰고, 커맨드는 내가 쳐서 부른다',
    source: D
  },
  {
    ...
    example: '"품의서 작성" 스킬을 만들어 두면 품의 관련 요청이 올 때 AI가 알아서 꺼내 쓴다',
  ...
  },
  {
    ...
    explain: '정해진 시점(도구 실행 전, 파일 수정 후, 세션 시작 등)에 자동으로 도는 처리다. 중요한 건 AI의 판단이 아니라 정해진 시점에 무조건 작동한다는 점이다. 그래서 안전장치로 쓴다.',
  ...
  },
  {
    ...
    myth: '"스킬의 다른 말"이 아니다. 스킬은 플러그인 안에 담기는 것 중 하나다',
    source: D
  },
  {
  ...
  },
  {
    ...
    explain: '계정에 추가해 둔 MCP 서버다. 내 컴퓨터에 직접 설정해 두는 것과 달리 계정을 따라다닌다. 회사가 조직 차원에서 미리 깔아 주고 도구별 권한을 걸 수도 있다.',
