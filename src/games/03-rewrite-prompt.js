...
export default {
  ...
  render(active, d = data) {
    ...
    return {
      ...
      say: `목표 ${res.total}줄 중 ${res.hit}줄이 맞았습니다.`
    };
  },
  ...
  named: {
    all: '방금 켠 것이 **프롬프트 엔지니어링**의 재료다 — 역할 · 맥락 · 형식 · 예시 · 제약. ' +
         '다만 “한 문단으로 짧게”와 “항목별로 나눠”는 서로의 효과를 지워서, 다 켜면 목표에서 오히려 멀어진다. ' +
         '맨 위의 역할을 매번 쓰지 않고 대화 전에 붙박이로 걸어 두면 그것이 **시스템 프롬프트**다.'
  },

  debrief: {
    pass: '목표 네 줄을 그대로 맞췄다. 켠 것은 넷이고 끈 것이 하나다.\n짧게 쓰라는 제약을 끄는 순간 각 줄에 원인이 다시 붙었다.\n잘 쓴 지시는 많이 넣은 지시가 아니라 서로 충돌하지 않는 지시다.',
    partial: '블록을 켜고 끌 때 목표와의 거리가 어느 쪽으로 움직였는지 다시 보라.\n형식과 제약을 같이 켜면 항목은 나뉘지만 각 줄에서 원인이 잘려 나간다.\n하나를 더 켜기 전에 지금 켠 것과 충돌하지 않는지부터 본다.',
    fail: '맥락을 안 준 결과를 다시 읽어 보라 — 틀린 말은 없는데 쓸 데가 없다.\n어떤 자료를 누구에게 왜 주는지가 빠지면 나머지를 아무리 다듬어도 일반론이 나온다.\n블록을 하나씩 켜면서 오른쪽 줄이 목표에 가까워지는지 멀어지는지 보라.'
  }
};
