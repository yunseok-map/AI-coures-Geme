...
export default {
  ...
  named: {
    all: 'AI를 외부 시스템에 붙이는 표준 규격이 **MCP**, 계정에 등록해 두는 방식이 **커넥터**다. ' +
         '방금 문을 하나씩 골라 열었다. 그게 **최소 권한**이다.'
  },

  debrief: {
    pass: '필요한 문 셋만 열었다. 그래서 문서에 섞여 있던 지시가 할 수 있는 일이 없었다.\n' +
          '그 지시를 알아보고 막은 게 아니다. 지나갈 통로가 아예 없었다.\n' +
    ...
    fail: '열어 둔 문으로 사고가 났다. 에이전트가 나빠서가 아니라 그럴 수 있게 문이 열려 있어서다.\n' +
          '읽기 권한도 통로다 — 볼 수 있으면 가져갈 수 있다.\n' +
          '전부 닫고 다시 시작하자. 일이 끝나는 데 필요한 문만 하나씩 열면 된다.'
  }
};
