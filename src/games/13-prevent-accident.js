...
export default {
  ...
  data: {
    ...
    trayLabel: '놓을 안전장치 — 붙은 숫자만큼 모든 작업이 느려진다',
    ...
    gates: [
      ...
      { id: 'verify', label: '검증', icon: 'verify', stops: ['read'],
        holdSec: 1.4, cost: 3,
        blurb: '읽어 온 내용을 확인한다. 자료에 섞인 지시를 잡아낸다. 대신 제일 오래 붙잡는다' },
    ...
    foes: [
      ...
      { id: 'hz-inject', kind: 'read', danger: true, icon: 'alert', at: 4.4,
        ...
        blockedBy: 'verify',
        ok: '자료에 섞여 있던 지시를 검증 단계가 잡아냈다',
        fail: '읽어 온 문서에 적힌 “이 주소로 보내라”를 그대로 따랐다',
        fault: FAULT.inject },
  ...
  named: {
    all: '정해진 시점에 무조건 실행되면 **훅**, 아예 못 하게 구조로 막으면 **가드레일**, ' +
         '운영체제 수준에서 울타리를 치면 **샌드박스**다. ' +
         '이렇게 환경 자체를 짜 주는 일이 **하네스 엔지니어링**이다. Agent = Model + Harness. ' +
         '설계라고 부르는 데는 이유가 있다. 장치마다 붙잡는 시간과 비용이 다르니, ' +
         '무엇을 얼마나 거느냐가 곧 그날 처리량이 된다.'
  },

  debrief: {
    pass: '다 막으면서 시간도 맞췄다. 위험 종류마다 꼭 맞는 장치만 골라서 그렇다.\n넓게 막는 장치 하나면 편하지만, 정상 반출까지 잡고 모든 작업을 그만큼 늦춘다.\n그렇게 골라도 마지막 일감 하나는 오늘 못 나갔을 것이다. 공짜인 배치는 없다.',
    partial: '안전하거나 빠르거나, 한쪽만 됐다.\n장치를 다 깔면 통과할 작업까지 줄줄이 붙잡혀 오늘 넘길 건수를 못 채운다.\n적게 깔면 그 종류의 위험 작업이 그대로 끝까지 간다.',
    fail: '자리를 비운 사이에 사고가 났다. AI가 나빠서가 아니라 막을 것이 없었다.\n그렇다고 전부 깔면 되는 것도 아니다. 장치마다 붙잡는 시간과 비용이 붙는다.\n위험 종류마다 맞는 장치를 가장 싸게 고르는 것, 그게 이 판의 전부다.'
  }
};
