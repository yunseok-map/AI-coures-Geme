...
export default {
  ...
  simulate(setup, d) {
    ...
    // 못 지킨 것 하나가 14점이다. **넷을 지키면 통과한다**(86점).
    //
    // 22점이던 때는 다섯을 전부 지켜야만 통과였다 — 하나만 놓쳐도 78점 조건부.
    // 이 판은 다섯 도구의 **발동 시점**을 처음 만나는 자리인데, 처음 배우는 것을
    // 다섯 개 한꺼번에 다 맞혀야 통과시키면 배우기 전에 막힌다.
    // 넷을 지켰다는 것은 이미 발동 시점을 잡았다는 뜻이고, 남은 하나는
    // 되짚기 화면이 어디였는지 그대로 짚어 준다.
    ...
    for (const g of missed) {
      ...
      if (g.want === 'laneBy' && laneEnd != null) {
        ...
        r.fault('늦은 발동', '따로 맡기긴 했지만 필요한 때보다 늦게 돌아왔다', 14);
        continue;
      }
      r.fault('발동하지 않은 일', g.text + ' — 오늘 이 자리에서 아무것도 발동하지 않았다', 14);
    }
