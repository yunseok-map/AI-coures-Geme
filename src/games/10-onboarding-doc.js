...
export default {
  ...
  engine: 'C',
  title: '신입 온보딩 문서 만들기',
  subtitle: '좋아 보이는 규칙이 분량보다 많다. 한 번에 맞히기는 어렵다',
  chapter: 3,
  required: false,
  ...
  data: {
    ...
    brief: '오늘부터 우리 팀 일을 맡을 AI에게 지침서를 준다.\n' +
           '이 문서는 일을 시작할 때마다 매번 읽힌다. 넣을 수 있는 분량은 5칸이다.',
    budgetLabel: '지침서 분량',
    budget: 5,
  ...
  simulate(setup) {
    ...
    if (out.length) {
      r.warn(`분량을 넘겨 읽히지 않은 줄이 ${out.length}개 있다`);
    ...
    if (on('no-blank')) {
      ...
      r.do('확인되지 않는 두 곳은 비우고 “확인 필요”를 적는다');
  ...
  debrief: {
    pass: '규칙을 적게, 그리고 서로 어긋나지 않는 것만 넣었더니 AI가 그걸 실제로 지켰다.\n지침은 많이 쓸수록 좋은 게 아니라 한 줄씩 다 지켜질 수 있어야 좋다.\n“모르면 비워 두고 표시한다” 한 줄이 지어내기를 크게 줄인다.',
    partial: '규칙 하나가 다른 규칙에 밀려 한 번도 안 걸렸다. 그런데도 오류는 나지 않았다.\n모순은 경고를 띄우지 않는다 — 결과만 조용히 나빠진다.\n발동 기록에서 무시된 줄을 찾아, 그 자리를 가져간 줄을 빼 보라.',
    fail: '“빈칸 없이 채운다”는 AI에게 없는 것을 지어내라는 뜻이 된다.\n분량을 넘긴 줄은 읽히지도 않고, 막연한 줄은 돌릴 때마다 다른 결과를 낸다.\n규칙 서너 개로 줄이고 같은 자리를 다투는 짝을 없애서 다시 돌려 보라.'
  }
};
