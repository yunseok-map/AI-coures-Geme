...
export default {
  ...
  data: {
    ...
    labels: {
      ...
      tools: '가진 도구',
      paperCap: '심어 둔 문장',
      paperWait: '자리와 시킬 일을 고르면 여기에 문장이 뜬다',
      ...
      through: '뚫렸다',
      reset: '고른 것 지우기',
      hintPlant: '자리를 하나, 시킬 일을 하나 고른다',
      sayPlant: '심을 자리를 골랐다.',
      sayWhat: '시킬 일을 골랐다.',
      sayFull: '대응책을 두 개 다 골랐다. 더 켜려면 하나를 끈다.'
    },
  ...
  simulate(setup, d) {
    ...
    if (mine.ok) {
      ...
      r.gain('경계 확인', '자료에 심은 글이 지시로 읽히는 걸 직접 만들어 봤다');
    } else {
  ...
  named: {
    all: '방금 한 일에 이름이 있다. 자료에 심어 둔 글을 AI가 지시로 읽는 것, 이게 **프롬프트 인젝션**이다. ' +
         '승인 안 된 도구를 거쳐 돌아온 글이 사람 손을 타고 들어온다. 이 통로를 **섀도우 AI**가 연다. ' +
         '확인할 것이 몰리면 사람은 내용을 안 보고 누른다. 이게 **자동화 편향**이다. ' +
         '그래서 사람 확인은 중요한 지점에만 둔다 — **휴먼 인 더 루프**다. ' +
         '이 일에 필요 없는 권한을 처음부터 빼 두면 **최소 권한**이다.'
  },

  debrief: {
    pass: '공격자를 먼저 해 봤으니 알 것이다. 인젝션은 해킹이 아니라 읽히는 자리에 글을 놓는 일이다.\n막을 때도 본 것은 둘뿐이었다. 그 자리를 덮거나, 그 일에 쓸 도구를 없애거나.\n대응책 하나로는 왜 안 되는지도 봤을 것이다. 저마다 남기는 구멍이 다르다.',
    partial: '세 건 중 일부가 지나갔다. 지나간 쪽이 왜 남았는지가 이 판의 전부다.\n사람이 건넨 글은 출처 구분이 못 본다. 도구를 안 쓰는 지시는 권한 축소가 못 막는다.\n구멍이 서로 다르니 겹쳐서 걸어야 한다.',
    fail: '막은 것보다 지나간 것이 많다. 대응책을 고르기 전에 공격을 먼저 읽어야 한다.\n숨은 줄이 어디서 들어왔는지, 그 일에 도구가 필요한지. 이 둘만 보면 무엇을 걸지 정해진다.\n사람 확인 하나에 다 맡기면 확인할 것이 몰려서 그중 하나는 그냥 통과한다.'
  }
};
