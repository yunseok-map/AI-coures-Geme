...
export default {
  ...
  data: {
    ...
    brief: `경쟁사 3곳을 각각 조사하고, 마지막에 하나의 표로 합쳐야 한다.\n` +
           `마감은 ${DEADLINE}칸. 취합은 조사가 끝난 뒤에 시작해야 한다.`,
  ...
  simulate(setup) {
    ...
    if (sched.end.merge == null) {
      ...
      r.fail(`${3 - feeding.length}개 회사가 빠진 채로 표가 만들어졌다`);
    } else {
  ...
  named: {
    all: '자기 책상을 따로 가진 일꾼에게 나눠 맡기는 것이 서브에이전트다. ' +
         '한 대화 안의 부하로 두느냐, 각자 독립된 자리를 주느냐 — 이 갈림이 ' +
         '**서브에이전트 vs 에이전트 팀**이고, 여럿의 순서와 합류를 설계하는 일이 **오케스트레이션**이다. ' +
         '방금 그은 선이 그 설계도다. 큰 작업을 옆방에서 돌려 본 대화를 깨끗하게 ' +
         '유지하는 것은 **컨텍스트 격리**라고 한다.'
  },

  debrief: {
    pass: '갈라졌다가 다시 모이는 선 — 그게 나눠 맡기기의 모양이다.\n독립된 일은 각자 시작하게 두고, 앞의 결과가 필요한 일만 선으로 묶으면 된다.\n실무에서 멀티 에이전트가 실패하는 지점은 대부분 “합치기”다.',
    partial: '선은 이어졌지만 마감을 넘었거나 취합에 빠진 조사가 있었다.\n한 줄로 이으면 안전한 대신 느리고, 걷어 오는 선을 빠뜨리면 빠른 대신 빈다.\n조사 셋을 서로 잇지 말고 각각 취합으로 이어 보라.',
    fail: '나눠 맡기는 건 공짜가 아니다. 시켜 놓고 걷어 오는 선을 안 그으면 결과는 흩어진 채 남는다.\n“이 일은 앞의 무엇이 끝나야 시작할 수 있나”만 보면 선이 정해진다.\n다시 이어 보라.'
  }
};
